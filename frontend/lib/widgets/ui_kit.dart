...
// ── Carte de trajet (Traject) ───────────────────────────────────────────────
class TrajectCard extends StatelessWidget {
  final String title;
  final String? subtitle;
  final String? priceLabel;    // ex: "200 DH"
  final String? durationLabel; // ex: "2h30"
  final double? rating;
  final bool verified;
  final Color accent;          // barre d'accent verticale
  final VoidCallback? onTap;
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        children: [
                          Expanded(
                            child: Text(title, style: AppTextStyles.titleMd, maxLines: 1, overflow: TextOverflow.ellipsis),
                          ),
                      ...
                      if (subtitle != null) ...[
                        const SizedBox(height: 4),
                        Text(subtitle!, style: AppTextStyles.bodySm, maxLines: 2, overflow: TextOverflow.ellipsis),
                      ],
                      const SizedBox(height: 12),
                      Row(
                        children: [
                          if (rating != null) ...[
                            const Icon(Icons.star, size: 15, color: AppColors.sand),
                            const SizedBox(width: 4),
                            Text(rating!.toStringAsFixed(1), style: AppTextStyles.bodySm.copyWith(fontWeight: FontWeight.w700, color: AppColors.ink)),
                            const SizedBox(width: 14),
                          ],
                          if (durationLabel != null) ...[
                            const Icon(Icons.schedule, size: 15, color: AppColors.textLight),
                            const SizedBox(width: 4),
                            Text(durationLabel!, style: AppTextStyles.bodySm),
                          ],
                          const Spacer(),
                          if (priceLabel != null)
                            Text(priceLabel!, style: AppTextStyles.titleMd.copyWith(color: AppColors.primary)),
                        ],
                      ),
                    ],
                  ),
